...
def grid_freqIosc_exp(idx):
	...
	I_oscs = np.linspace(I_osc_lims[0], I_osc_lims[1], res) * 1e-12
	fs = np.linspace(f_lims[0], f_lims[1], res)

	experiments = list(itertools.product(I_oscs, fs))
	...
	filename = 'Data_Iosc/' + 'experimental_' + '{0:03d}'.format(int(idx/res)) + '_' + '{0:03d}'.format(int(idx%res)) + ".pickle"
