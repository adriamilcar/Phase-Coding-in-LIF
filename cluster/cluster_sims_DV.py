...
def grid_freqDV_exp(idx):
	...
	V_th_DV = np.linspace(20.22, 15.78, res) * 1e-3
	R_m_DV = np.linspace(24.8, 94.4, res) * 1e6
	tau_m_DV = np.linspace(14.45, 33.35, res) * 1e-3
	I_osc_DV = np.linspace(60,30, res) * 1e-12
	DV_gradient = np.arange(res)

	experiments = list(itertools.product(DV_gradient, fs))
	num_experiments = len(list(experiments))
	DV_indx, f = experiments[idx]

	# Get params
	R_m = R_m_DV[DV_indx]
	V_th = V_th_DV[DV_indx]
	tau_m = tau_m_DV[DV_indx]
	I_osc = I_osc_DV[DV_indx]
	...
	filename = 'Data_DV/' + 'experimental_' + '{0:03d}'.format(int(idx/res)) + '_' + '{0:03d}'.format(int(idx%res)) + ".pickle"
